...
You're going to forge the squad's most powerful skill: **`/recruit-squad-member`** —
a one-command scaffold that creates a whole new teammate (roster entry + mascot art
...
Create **`.github/prompts/recruit-squad-member.prompt.md`**.
...
description: Scaffold a brand-new squad member — roster entry, mascot art, agent file
...
2. **Add a roster entry** to `src/squad.ts` — a full `SquadMember` object, every
   field filled, with a catchphrase and 4 quips in the new member's voice.
3. **Handle mascot art** in `src/mascots.ts` — reuse an existing animal's builder,
...
2. In Copilot Chat, type `/` — you should see **`recruit-squad-member`** in the
...
- [ ] `.github/prompts/recruit-squad-member.prompt.md` exists
- [ ] It uses `${input:...}` variables for the new member's details
- [ ] `/recruit-squad-member` appears in the Copilot Chat `/` menu

## Stuck? Peek at the answer key

```bash
git show solution:.github/prompts/recruit-squad-member.prompt.md
```

---

➡️ **Next:** [LAB 4 — Assemble the Squad](LAB-4-the-squad.md). Time to build the
Captain and watch the whole squad work together — live.
